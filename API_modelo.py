...
class Prediction(Base):
    ...
    sepal_lenght = Column(Float, nullable=False)
...
def predict():
    data = request.get_json(force=True)
    try:
        sepal_lenght = float(data["sepal_length"])
        sepal_width = float(data["sepal_width"])
        petal_lenght = float(data["petal_length"])
